...
class K_IncDBSCANISBGD_reduced(BISGD):
    ...
    def IncrTrain(self, user, item, update_users: bool = True, update_items: bool = True):
        ...
        if(self.iterator < self.k):
            ...
            print("length user factors")
            print(len(self.user_factors[0]))
            print("users maxid")
            print(self.data.maxuserid)
            print("length meta user")
            print(len(self.metamodel_users))
            print("___________________________________________________________________")"""
            ...
            if len(self.user_factors[0])  == self.data.maxuserid :
                ...
                """
                print("bishshshs")
                print(np.abs(np.random.normal(0.0, 0.1, self.num_nodes)))
                print("metamodel length users")
                print(len(self.metamodel_users))
                print("updated metamodel users")"""
                #self.metamodel_users.append(np.abs(np.random.normal(0.0, 0.1, self.num_nodes)))
                
                for node in range(self.k):
                    #self.user_factors[node] = np.append(self.user_factors[node], np.random.normal(0.0, 0.1, self.num_factors))
                    self.user_factors[node].append(np.random.normal(0.0, 0.1, self.num_factors))
                    """
                    print("brushshshhs")
                    print(np.random.normal(0.0, 0.1, self.num_factors))
                    print("user factor length")
                    print(len(self.user_factors[node]))
                    print("updated user factor users")
            print("___________________________________________________________________")
            print("length item factors")
            print(len(self.item_factors[0]))
            print("item maxid")
            print(self.data.maxitemid)
            print("___________________________________________________________________")"""
            if len(self.item_factors[0]) == self.data.maxitemid:
                self.metamodel_items.append(np.abs(np.random.normal(0.0, 0.1, self.num_nodes)))
                """
                print("metamodel length items")
                print(len(self.metamodel_items))
                print("updated metamodel items")"""
                #self.metamodel_items.append(np.abs(np.random.normal(0.0, 0.1, self.num_nodes)))
                for node in range(self.k):
                    ...
                    """
                    print("item factor length")
                    print(len(self.item_factors[node]))
                    print("updated item factor")"""
            ...
            #print('Execution time incremental add:', elapsed_time, 'seconds')
            #print("_______________________________________________________________________________________________________________________________________________________________")
            ...
            for clust in self.Clusters:
                for core in self.curCores:
                    current_core = [core[0],core[1]]
                    if clust.has(current_core):
                        core_user = core[0]
                        self.Dic[clust.name][core_user] = 0
                        u = self.data.GetUserInternalId(core_user)
                        ...
                        for point in self.dataSet:
                            user = point[0]
                            if(user!=core_user):
                                temp_user = self.data.GetUserInternalId(user)
                                temp_user_vector = self.metamodel_users[temp_user]
                                dist = np.linalg.norm(user_vector - temp_user_vector)
                                self.Dic[clust.name][user] = dist"""
            
            """
            temp_dic ={}                    
            for clust in self.Dic:
                value = self.Dic[clust][user]
                clust_number = int(clust.replace("Cluster", ""))
                temp_dic[clust_number] = value
            """
            #clusters_sort = self.Clusters.sort()
            #print("Sorted")
            #print(clusters_sort)
    ...
    def incrementalAdd(
        ...
        elapsed_time = et - st
        #print("_______________________________________________________________________________________________________________________________________________________________")
        #print('Execution time visit neighbors of p:', elapsed_time, 'seconds')    
        #print("_______________________________________________________________________________________________________________________________________________________________")
        #for each p of these new cores
        st = time.time()
        for core in self.newCores:
