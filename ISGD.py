...
class ISGD(Model):
    ...
    def IncrTrain(self, user, item, update_users: bool = True, update_items: bool = True, n_times: int = 1):
        ...
        """
        print("estou no incremental training")
        user_id, item_id = self.data.AddFeedback(user, item)
        print("lenght user factors")
        ...
        if update_users or update_items:
            print("its update users or items")
    ...
    def Predict(self, user_id, item_id):
        ...
        item_id -- The ID of the item
        """
        print(np.inner(self.user_factors[user_id], self.item_factors[item_id]))
